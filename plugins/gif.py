...
def gif(searchterm, unsafe=False):
    ...
    searchurl = "https://www.google.com/search?tbs=itp:animated,ift:gif&tbm=isch&q={0}{1}".format(searchterm, safe)
    ...
    return (gifs[0].split(".gif")[0] + ".gif") if gifs else ""

def on_message(msg, server):
    text = msg.get("text", "")
    match = re.findall(r"bill gif (.*)", text)
    if not match: return
